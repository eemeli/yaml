...
function parseMap(doc, ast) {
  var _ref = ast.type === _Node.Type.FLOW_MAP ? resolveFlowMapItems(doc, ast) : resolveBlockMapItems(doc, ast),
  ...
  for (var i = 0; i < items.length; ++i) {
    ...
    for (var j = i + 1; j < items.length; ++j) {
      ...
      if (iKey === jKey || iKey && jKey && iKey.hasOwnProperty('value') && iKey.value === jKey.value) {
        doc.errors.push(new _errors.YAMLSemanticError(ast, "Map keys must be unique; \"".concat(iKey, "\" is repeated")));
        break;
      }
    }
  ...
  for (var i = 0; i < ast.items.length; ++i) {
    ...
    switch (item.type) {
      ...
      case _Node.Type.MAP_VALUE:
        ...
        (0, _parseUtils.checkKeyLength)(doc.errors, ast, i, key, keyStart);
        key = undefined;
        keyStart = null;
        break;
...
function resolveFlowMapItems(doc, ast) {
  ...
  for (var i = 0; i < ast.items.length; ++i) {
    ...
    if (typeof item === 'string') {
      ...
      doc.errors.push(new _errors.YAMLSyntaxError(ast, "Flow map contains an unexpected ".concat(item)));
  ...
  if (ast.items[ast.items.length - 1] !== '}') doc.errors.push(new _errors.YAMLSemanticError(ast, 'Expected flow map to end with }'));
  if (key !== undefined) items.push(new _Pair.default(key));
  return {
    comments: comments,
    items: items
